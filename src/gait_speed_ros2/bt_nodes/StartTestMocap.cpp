...
StartTest::tick()
{
    ...
    try
    {
        ...
        // Call to mocap service
        rclcpp::Client<std_srvs::srv::Empty>::SharedPtr mocap_start_client;
        mocap_start_client = node_->create_client<std_srvs::srv::Empty>("start_gait_speed");

        while (!mocap_start_client->wait_for_service(std::chrono::seconds(1))) {
            RCLCPP_WARN(node_->get_logger(), "Waiting for mocap service to be available...");
        }

        auto request = std::make_shared<std_srvs::srv::Empty::Request>();
        auto result_future = mocap_start_client->async_send_request(request);
        
        if (rclcpp::spin_until_future_complete(node_->get_node_base_interface(), result_future) ==
            rclcpp::FutureReturnCode::SUCCESS) {
            RCLCPP_INFO(node_->get_logger(), "Mocap service executed successfully");
        } else {
            RCLCPP_ERROR(node_->get_logger(), "Mocap service error.");
        }
        // Call to mocap service
...
BT_REGISTER_NODES(factory) {
  factory.registerNodeType<gait_speed::StartTest>("StartTestMocap");
}
